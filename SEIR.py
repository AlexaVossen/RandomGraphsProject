...
def one_it(G,p_1,p_2, p_3,I_s,S_c,S_t,S_p,E, tot_inf, always_infected = True):
    """
    ...
    for node in S_p:
        ...
        if always_infected:
            ...
            if prob>=1:
                ...
                i = np.random.binomial(1, prob)
        if i == 1:
            S_p_new.remove(node)
            E_new.append(node)
    return I_s_new,S_c_new,S_t_new,S_p_new,E_new, tot_inf

def k_it(k,G,p_1,p_2, p_3,I_s,S_c,S_t,S_p,E):
    "Given a graph and nodes in each state, performs state transitions until stationary"
    tot_inf = 1
    n = 1000
    tot_inf_list = [tot_inf/n]
    i_s = [len(I_s)]
    s_c = [len(S_c)]
    s_t = [len(S_t)]
    s_p = [len(S_p)]
    ...
    while cond:
        if i > k:
            cond = False
        I_s,S_c,S_t,S_p,E,tot_inf = one_it(G,p_1,p_2, p_3,I_s,S_c,S_t,S_p,E,tot_inf)
        tot_inf_list.append(tot_inf/n)
        if not I_s and not E:
            ...
            if seen:
                cond = False
        i+=1
    return i_s,s_c,s_t,s_p,e,tot_inf/n,tot_inf_list
p_1 = 1
p_2 = 0.25
p_3 = 1
...
def simulations(it, graphs, p_1,p_2,p_3):
    ...
    for G in graphs:
        for i in range(it):
            ...
            S_p = V.copy()
            S_p.remove(start)
            a,b,c,d,e,tot_inf, inf = k_it(k,G, p_1, p_2, p_3, I_s, S_c, S_t, S_p, E)
            L_inf.append(inf)
            L_death_time.append(len(inf)-1)
            L_tot_inf.append(tot_inf)
    return L_inf, L_death_time ,L_tot_inf
...
#Loop over degrees and probabilities to genrate results which are saved as figures and data
for d in degrees:
    if d == 20:
        for p in probabilities:
            graphs = load_graphs(f'graphs_{d}.pickle')
            L_inf, L_death_time, L_tot_inf = simulations(100,graphs, p_1,p,p_3)
            ltotinf = confidence_interval(L_tot_inf)
            ldeathtime = confidence_interval(L_death_time)
            deel3 = CI_over_time(L_inf)
            f = open(f'data/SEIR_a_deg_{d}_prob_{p}.txt', "w")
            f.write('CI yield:')
            ...
            ax1.set_title(f'Fraction of infected nodes per time step with p_2={p} and E[D]={d}')
            ...
            fig.tight_layout()
            fig.savefig(f'plots/SEIR_a_deg_{d}_prob_{p}.png')
    else:
        graphs = load_graphs(f'graphs_{d}.pickle')
        L_inf, L_death_time, L_tot_inf = simulations(100,graphs, p_1,0.25,p_3)
        ltotinf = confidence_interval(L_tot_inf)
        ldeathtime = confidence_interval(L_death_time)
        deel3 = CI_over_time(L_inf)
        ...
        ax1.plot(mean_list, linestyle='None', marker='o', label = "mean CI") 
        ax1.plot(upper_list, linestyle='None', marker='o', label = "upper bounds CI") 
        ax1.set_title(f'Fraction of infected nodes per time step with p_2=0.25 and E[D]={d}')
        ax1.set( ylabel="Fraction of infected nodes")
        ax2.set(xlabel="Time steps",ylabel='Number of occurrences')
        ax1.legend() 
        ax2.hist(L_death_time, bins = [i for i in range(max(L_death_time)+1)], color='skyblue', edgecolor='black')
        ax2.set_title('Histogram for the time till death of each run')
        fig.tight_layout()
        fig.savefig(f'plots/SEIR_deg_{d}_prob_0.25.png')
